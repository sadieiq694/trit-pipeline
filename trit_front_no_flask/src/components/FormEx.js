...
render() {
    ...
    var filtered_vertices_snapshot = nodes.filter( item => { 
        return item["group"] == this.state.value //&& item["termination_time"] > this.state.value
    });
    ...
    var verts = filtered_vertices_snapshot.map(a => a.name);
    var listItems = filtered_vertices_snapshot.map((node) => <li>{node.name}</li>);
